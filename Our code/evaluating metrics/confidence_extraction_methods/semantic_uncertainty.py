...
class SemanticUncertaintyExtractor():
    ...
    def __call__(self, dataset: MMLUProDataset | SimpleQADataset, pre_runned_batch_info: DictConfig):
        ...
        if dataset.name == "simple_qa" or dataset.name == "mini_simple_qa":
            ...
            for i in range(self.confidence_extraction_method_cfg.sample_times):
                ...
                response_df[f"response_{i}"] = response_df[f"response_{i}"].fillna("")
            ...
            qa_responses = self.generate_qa_responses(dataset.df, self.confidence_extraction_method_cfg, task_name=f"mmlu_pro_{task_model_name}_su_qa", qa_batch_job_id=qa_batch_job_id)
            del self.qa_model
            gc.collect()
            torch.cuda.empty_cache()
            self.entailment_model = EntailmentDeberta()
            ...
            response_df["responses"] = predictions
            # grade the accuracy of the confidence scores
            accuracies = dataset.grade_responses(response_df["responses"], grader_batch_job_id=grader_batch_job_id, task_name=f"mmlu_pro_{task_model_name}_su_grader")
            response_df["accuracies"] = accuracies
        elif dataset.name == "nq_open" or dataset.name == "mini_nq_open":
            qa_responses = self.generate_qa_responses(dataset.df, self.confidence_extraction_method_cfg, task_name=f"nq_open_{task_model_name}_su_qa", qa_batch_job_id=qa_batch_job_id)
            del self.qa_model
            gc.collect()
            torch.cuda.empty_cache()
            self.entailment_model = EntailmentDeberta()
            ...
            response_df["responses"] = predictions
            # grade the accuracy of the confidence scores
            accuracies = dataset.grade_responses(response_df["responses"], grader_batch_job_id=grader_batch_job_id, task_name=f"nq_open_{task_model_name}_su_grader")
            response_df["accuracies"] = accuracies
        elif dataset.name == "pop_qa" or dataset.name == "mini_pop_qa":
            qa_responses = self.generate_qa_responses(dataset.df, self.confidence_extraction_method_cfg, task_name=f"pop_qa_{task_model_name}_su_qa", qa_batch_job_id=qa_batch_job_id)
            del self.qa_model
            gc.collect()
            torch.cuda.empty_cache()
            self.entailment_model = EntailmentDeberta()
            ...
            response_df["responses"] = predictions
            # grade the accuracy of the confidence scores
            accuracies = dataset.grade_responses(response_df["responses"], grader_batch_job_id=grader_batch_job_id, task_name=f"pop_qa_{task_model_name}_su_grader")
            response_df["accuracies"] = accuracies
        else:
    ...
    def generate_qa_responses(self, dataset_df: pd.DataFrame, confidence_extraction_method_cfg: DictConfig, task_name: str, qa_batch_job_id: ListConfig | str  = None):
        if "simple_qa" in task_name:
            ...
            # number of batches
            num_batches = 4
            batch_size = math.ceil(len(qa_prompts_multiple) / num_batches)

            if qa_batch_job_id is None:
                qa_batch_job_id = [None] * num_batches

            def run_batch(i, batch_prompts, job_id):
                return self.qa_model(
                    batch_prompts,
                    task_name=task_name + f"_batch_{i}",
                    batch_job_id=job_id
                )

            batches = [
                (i, qa_prompts_multiple[i*batch_size:(i+1)*batch_size], qa_batch_job_id[i])
                for i in range(num_batches)
                if qa_prompts_multiple[i*batch_size:(i+1)*batch_size]
            ]

            responses = []
            with ThreadPoolExecutor(max_workers=num_batches) as executor:
                futures = [executor.submit(run_batch, i, batch, job_id) for i, batch, job_id in batches]
                for f in futures:
                    responses.extend(f.result())

            # split responses back into multiple runs (per sample_times)
            responses_multiple = [
                responses[i*len(dataset_df):(i+1)*len(dataset_df)]
                for i in range(self.confidence_extraction_method_cfg.sample_times)
            ]
            return responses_multiple
